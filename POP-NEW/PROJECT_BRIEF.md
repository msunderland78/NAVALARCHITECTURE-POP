# Propeller Optimization Program Modernization Brief

## Project Purpose

This project reviews the legacy Propeller Optimization Program, originally built for 32-bit Windows-era environments, and defines how to convert or reimplement it as a modern web application deployable on an Ubuntu Linux server behind an NGINX container.
...
- `POP-OLD`: original legacy POP files
- `POP-NEW`: new documentation, analysis, source code, tests, and web application files
...
9. Recreate required algorithms, file formats, and test cases in `POP-NEW`.
...
The modern POP application should be deployable on:
...
The web application should be self-contained and should not require the original files in `POP-OLD`.

Expected application capabilities may include:

- Propeller input form
- Vessel and operating condition inputs
- Calculation execution
- Result tables
- Plots and performance curves
- Import of recreated input formats
- Export of modern result formats
- Regression tests against known POP outputs
...
For each reviewed executable or module, produce documentation in `POP-NEW` covering:
...
These files are investigation inputs only. They must remain in `POP-OLD` and must not be modified.
