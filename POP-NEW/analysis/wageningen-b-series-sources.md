...
In optimization mode the implementation sweeps blade count `Z in {3, 4, 5, 6, 7}` and reports both the global best design and the per-Z optimum. Legacy POP-1.5 only optimized over `(D, P/D, Ae/Ao)` at the user-selected `Z`. The per-Z table makes early-stage blade count selection an output of the calculation rather than a separate user decision.
